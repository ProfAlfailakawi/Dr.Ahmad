...
export class AgentDatabase {
  ...
  purgeExpired(days = RETENTION_DAYS) { this.run("DELETE FROM intent_logs WHERE created_at < datetime('now', ?)", `-${days} days`); this.run("DELETE FROM unresolved_messages WHERE created_at < datetime('now', ?)", `-${days} days`); this.run("DELETE FROM message_attempts WHERE attempted_at < datetime('now', ?)", `-${days} days`) }
