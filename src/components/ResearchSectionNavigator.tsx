    ...
    >
      <nav className="mx-auto max-w-[960px] rounded-2xl border border-hair bg-paper p-1.5" aria-label="التنقل داخل البحث">
        ...
        {levels.map((level, index) => (
          <button key={level.key} type="button" onClick={() => onSelect(level.key)} aria-current={active === level.key ? 'step' : undefined} className={`min-h-[3.55rem] rounded-xl border border-transparent px-2 py-2.5 text-center transition-colors duration-100 sm:px-4 ${active === level.key ? 'border-accent bg-accent text-white' : 'text-soft hover:bg-wash hover:text-ink'}`}>
