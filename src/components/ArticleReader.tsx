  ...
  try {
    ...
    const scored = candidates
      .map((item) => {
        ...
        // يتقاطعان في حدود الفقرة ذاتها وبنسبة اختلاف لا تتجاوز 40% زيادة أو نقصاناً
        const hasIntersection = Math.max(startOffset, item.start) < Math.min(endOffset, item.end)
        const withinFortyPercent = lenDiffRatio <= 0.40 && (spans > 0.15 || words > 0.20 || hasIntersection)

        return { item, spans, words, lenDiffRatio, withinFortyPercent }
      })
      .filter(({ spans, words, withinFortyPercent }) => withinFortyPercent || (spans >= .35 && words >= .40))
      .sort((left, right) => {
