...
export function searchContent(db, query, options = {}) {
  ...
  let rows = []
  try {
    const match = text.split(/\s+/).filter(Boolean).map((token) => `"${token.replace(/"/g, '')}"*`).join(' AND ')
    rows = db.all('SELECT content_items.*, bm25(content_items_fts) AS rank FROM content_items_fts JOIN content_items ON content_items.id=content_items_fts.id WHERE content_items_fts MATCH ? ORDER BY rank LIMIT ?', match, limit)
  } catch { rows = [] }
  if (!rows.length) {
    const like = `%${text.replace(/[%_]/g, '')}%`
    rows = db.all('SELECT * FROM content_items WHERE lower(title||\' \'||excerpt||\' \'||body||\' \'||keywords) LIKE ? ORDER BY date DESC LIMIT ?', like, limit)
  }
