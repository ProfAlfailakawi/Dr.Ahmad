...
function YouTubeThumb({ videoId, title }: { videoId: string; title: string }) {
  const [source, setSource] = useState(() => `https://i.ytimg.com/vi/${videoId}/hqdefault.jpg`)
  ...
  return (
    <img decoding="async"
      ...
      loading="lazy"
      onLoad={(event) => {
        if (event.currentTarget.naturalWidth <= 120 && !source.includes('/mqdefault.')) setSource(`https://i.ytimg.com/vi/${videoId}/mqdefault.jpg`)
      }}
      onError={() => {
        if (!source.includes('/mqdefault.')) setSource(`https://i.ytimg.com/vi/${videoId}/mqdefault.jpg`)
        else setHidden(true)
      }}
      className="selected-media-thumb h-full w-full object-cover transition-opacity duration-300 group-hover:opacity-95"
    />
  )
}
    ...
    <section className="border-t border-hair bg-wash px-6 py-[58px] md:px-11 md:py-[96px]">
      <div className="mx-auto max-w-shell">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {book && <FadeUp><Link to={`/publications/${book.slug}`} className="group grid min-h-[230px] grid-cols-[.7fr_1.3fr] overflow-hidden rounded-2xl border border-hair bg-canvas transition-colors hover:border-accent"><div className="overflow-hidden bg-white"><img decoding="async" src={book.cover} srcSet={coverSrcSet(book.cover)} sizes="(max-width: 640px) 40vw, 300px" alt={book.title} loading="lazy" className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" /></div><div className="flex flex-col justify-between p-6"><div><p className="text-[.74rem] font-semibold text-accent">كتاب مختار</p><h3 className="mt-3 font-display text-[1.18rem] font-semibold leading-[1.55] text-ink group-hover:text-accent">{book.title}</h3></div></div></Link></FadeUp>}
          {paper && <FadeUp delay={.06}><Link to={`/research/${paper.slug}`} className="group flex min-h-[230px] flex-col justify-between rounded-2xl border border-hair bg-canvas p-7 transition-colors hover:border-accent"><div><p className="text-[.74rem] font-semibold text-accent">بحث محكّم</p><h3 className="mt-4 font-display text-[1.25rem] font-semibold leading-[1.65] text-ink group-hover:text-accent">{paper.title}</h3><p className="mt-3 text-[.8rem] leading-relaxed text-soft">{paper.meta}</p></div></Link></FadeUp>}
          {article && <FadeUp delay={.1}><Link to={`/articles/${article.slug}`} className="group flex min-h-[215px] flex-col justify-between rounded-2xl border border-hair bg-canvas p-7 transition-colors hover:border-accent"><div><p className="text-[.74rem] font-semibold text-accent">مقال بصوتي</p><h3 className="mt-4 font-display text-[1.25rem] font-semibold leading-[1.65] text-ink group-hover:text-accent">{article.title}</h3></div></Link></FadeUp>}
          {appearance && <FadeUp delay={.14}><Link to={appearance.slug ? `/media/${appearance.slug}` : '/media'} className="group grid min-h-[215px] grid-rows-[1fr_auto] overflow-hidden rounded-2xl border border-hair bg-canvas transition-colors hover:border-accent"><div className="selected-media-frame relative min-h-[145px] overflow-hidden bg-ink/10" style={{ '--media-thumb': ytId(appearance.url) ? `url(https://i.ytimg.com/vi/${ytId(appearance.url)}/hqdefault.jpg)` : 'none' } as CSSProperties}>{ytId(appearance.url) && <YouTubeThumb videoId={ytId(appearance.url)} title={appearance.title} />}<span className="absolute inset-0 flex items-center justify-center bg-ink/10"><span className="flex h-12 w-12 items-center justify-center rounded-full bg-canvas/90 text-accent"><SocialIcon name="Play" size={20} /></span></span></div><div className="p-5"><p className="text-[.72rem] font-semibold text-accent">{appearance.outlet}</p><h3 className="mt-1 font-display text-[1rem] font-semibold leading-[1.55] text-ink group-hover:text-accent">{appearance.title}</h3></div></Link></FadeUp>}
        </div>
