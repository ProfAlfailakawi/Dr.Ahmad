...
export function ProductionMonitor({articles,onOpen}:Props){
  ...
  const rows=[
    ['التصميم','التصاميم والأصول المولّدة',stats.designs,'design'],
    ['الحملات','خطط النشر المتكاملة',stats.campaigns,'design'],
    ['الصوت','جاهز / ينتظر',`${stats.audioReady} / ${stats.audioPending}`,'sound-caravan'],
  ...
  return <div className="grid gap-5">
    <section className={`${card} border-accent/25 bg-accent/[.035]`}>
      <p className="text-[.72rem] font-bold uppercase tracking-[.16em] text-accent">مراقب الإنتاج</p>
      <div className="mt-2 flex flex-wrap items-end justify-between gap-4"><div><h2 className="font-display text-[clamp(1.5rem,3vw,2.3rem)] font-bold text-ink">حالة المنظومة الآن.</h2><p className="mt-2 max-w-2xl text-[.82rem] leading-relaxed text-soft">تقرأ الحالة من الأعلى، ثم تفتح الأداة الأصلية عند الحاجة؛ القرارات اليومية تبقى في غرفة القيادة.</p></div><span className={`rounded-full px-4 py-2 text-[.74rem] font-semibold ${stats.guardian==='healthy'?'bg-accent/10 text-accent':'bg-amber-50 text-amber-900'}`}>{stats.guardian==='healthy'?'الحارس سليم':'الحارس يحتاج فحصاً'}</span></div>
    </section>
