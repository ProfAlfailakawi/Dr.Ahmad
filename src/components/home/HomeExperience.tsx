    ...
    <section className="border-t border-hair bg-wash px-6 py-[58px] md:px-11 md:py-[96px]">
      <div className="mx-auto max-w-shell">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          {appearance && <FadeUp delay={.14}><a href={appearance.url} target="_blank" rel="noreferrer" className="group grid min-h-[215px] grid-rows-[1fr_auto] overflow-hidden rounded-2xl border border-hair bg-canvas transition-colors hover:border-accent"><div className="relative min-h-[145px] overflow-hidden bg-ink/10">{ytId(appearance.url) && <img src={`https://i.ytimg.com/vi/${ytId(appearance.url)}/hqdefault.jpg`} alt={appearance.title} loading="lazy" className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" />}<span className="absolute inset-0 flex items-center justify-center bg-ink/15"><span className="flex h-12 w-12 items-center justify-center rounded-full bg-canvas/90 text-accent">▶</span></span></div><div className="p-5"><p className="text-[.72rem] font-semibold text-accent">{appearance.outlet}</p><h3 className="mt-1 font-display text-[1rem] font-semibold leading-[1.55] text-ink group-hover:text-accent">{appearance.title}</h3></div></a></FadeUp>}
        </div>
    ...
    <section className="border-t border-hair px-6 py-[58px] md:px-11 md:py-[88px]">
      <div className="mx-auto max-w-shell">
        ...
        <div className="mt-10 grid gap-8 md:grid-cols-[1.1fr_.9fr]">
          ...
          <FadeUp delay={.08}><div className="rounded-2xl border border-hair bg-wash p-6"><p className="text-[.75rem] font-semibold text-accent">الخاتمة الهادئة</p><p className="mt-3 font-display text-[1.25rem] font-semibold leading-[1.75] text-ink">هذا الموقع لا يعرض أرشيفاً فقط؛ إنه يفتح طريقاً إلى الفكرة.</p><div className="mt-5 flex flex-wrap gap-3"><Link to="/contact" className="rounded-full bg-accent px-5 py-2.5 text-[.84rem] font-semibold text-white">تواصل معي</Link><Link to="/about" className="rounded-full border border-hair px-5 py-2.5 text-[.84rem] font-semibold text-soft hover:border-accent hover:text-accent">حول الموقع</Link></div></div></FadeUp>
        </div>
      </div>
    </section>
  )
