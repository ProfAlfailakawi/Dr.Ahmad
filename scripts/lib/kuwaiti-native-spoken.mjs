...
const PILOT_TURN_OVERRIDES = new Map([
  ...
  [15, { speaker: 'female', text: 'في دراسة كبيرة فيها أبحاث وايد عن الموضوع.' }],
...
const PILOT_FULL_OVERRIDES = new Map([
  ...
  [10, { speaker: 'female', text: 'في دراسة كبيرة فيها أبحاث وايد عن الموضوع، وطلع معاهم إن كل ما زاد التوتر، نزل مستوى الطالب.' }],
...
const SAFE_TEXT_RULES = [
  ...
  [/^مراجعة بحثية واسعة، لقت إن المستخدمين يقبلون توصيات الذكاء حتى وهي ناقصة\.$/u,
    'وفي مراجعة فيها أبحاث وايد، طلع إن الناس تمشي ورا توصيات الذكاء حتى لما تكون ناقصة.'],
...
export function optimizeNativeSpokenEpisode (turns, { slug = '' } = {}) {
  ...
  if (slug === PILOT_SLUG && output.length === 27
    ...
    const mergedResearch = [
      ...
      {
        ...output[15],
        text: 'في دراسة كبيرة فيها أبحاث وايد عن الموضوع. كل ما زاد التوتر، نزل مستوى الطالب.',
