 ...
 *
 * الفكرة: متن الـ١٤٤ هو **خط أساس معروف**، لا شهادةٌ كاذبة بأن كل مفردة
 * فيه مرت بأذن الدكتور. الكلمات عالية الخطر منه تمر في مختبر المعجم
 * الطويل، وأي كلمة يجلبها مقالٌ جديد تقف هنا قبل Gemini:
...
/* مفردات خط الأساس: متن الحلقات الـ١٤٤ (المكتوب والمنطوق معاً) + مفاتيح
   ...
   «معتمدة سمعياً»؛ الاعتماد السمعي منفصل في heardByEar ودفتر المختبر. */
...
function report(findings) {
  if (!findings.length) { console.log('✅ حارس الكلمات الجديدة: كل مفردات الحوار ضمن خط الأساس — لا كلمة مستقبلية جديدة تحتاج أذن الدكتور.'); return }
  ...
  console.log('\nالعلاج: تدخل الكلمات الجديدة بجملها إلى مختبر المعجم الطويل')
  console.log('(scripts/podcast-kuwaiti-lexicon-screening.mjs)، يسمعها الدكتور، ثم تدخل المعجم ويخضر الحارس.')
