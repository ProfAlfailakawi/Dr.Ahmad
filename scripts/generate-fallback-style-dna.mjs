...
lines.push(`  recent: { sample: ${d.recent.sample}, retired: [${d.recent.retired.map((w) => `'${w}'`).join(', ')}], semicolonShare: ${num(d.recent.semicolonShare)}, askYourselfShare: ${num(d.recent.askYourselfShare)}, perhapsBeginsShare: ${num(d.recent.perhapsBeginsShare)}, citationShare: ${num(d.recent.citationShare)}, citationsPerArticle: ${d.recent.citationsPerArticle ? `{ p50: ${d.recent.citationsPerArticle.p50}, p85: ${d.recent.citationsPerArticle.p85} }` : 'null'}, closingAntithesisShare: ${num(d.recent.closingAntithesisShare)}, closingQuestionShare: ${num(d.recent.closingQuestionShare)}, sentenceP90: ${d.recent.sentenceP90}, tanweenPer1000: ${d.recent.tanweenPer1000 ? `{ p50: ${d.recent.tanweenPer1000.p50}, p85: ${d.recent.tanweenPer1000.p85} }` : 'null'}, longShare: ${num(d.recent.longShare)}, rareFormulas: [${d.recent.rareFormulas.map((w) => `'${w}'`).join(', ')}], openers: [${d.recent.openers.map((w) => `'${w}'`).join(', ')}], paragraphsMedian: ${d.recent.paragraphsMedian}, paragraphsP75: ${d.recent.paragraphsP75}, paragraphWordsMedian: ${d.recent.paragraphWordsMedian}, openingShares: { ${Object.entries(d.recent.openingShares).map(([k, v]) => `${k}: ${num(v)}`).join(', ')} }, openingPauseShare: ${num(d.recent.openingPauseShare)}, bands: { ${Object.entries(d.recent.bands).map(([key, band]) => `${key}: { ${Object.entries(band).map(([p, v]) => `${p}: ${num(v)}`).join(', ')} }`).join(', ')} } },`)
