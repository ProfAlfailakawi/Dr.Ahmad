...
check('لا تُقدَّم الرسائل المصطنعة بوصفها رسائل حقيقية', inbox.includes('وليست رسائل واردة من أشخاص حقيقيين') && inbox.includes('نموذج رسالة تمثيلية'))
check('Flow يوفّر خيار مع كلام وخياراً صامتاً ببرومبت إنجليزي فقط', liveDirector.includes("FlowPromptMode = 'speech' | 'silent'") && liveDirector.includes('buildFlowPrompts') && liveDirector.includes('englishOnly') && liveDirectorUi.includes('مع كلام') && liveDirectorUi.includes('بدون كلام') && liveDirectorUi.includes('getFlowPrompt'))
check('الجملة الصوتية منفصلة ولا يدخل النص العربي إلى برومبت Flow', liveDirector.includes('The exact spoken line is supplied separately') && liveDirector.includes('ARABIC_SCRIPT') && liveDirector.includes('Arabic writing') && liveDirectorUi.includes('الجملة الصوتية لا تدخل داخل البرومبت'))
