...
function perfectArticleInput(value) {
  ...
  /* مادة الدكتور لهذا المقال: مناسبته، أو موقفٌ عاشه، أو جملةٌ سمعها، أو دراسةٌ برقمها.
     الحَكَم الأعمى فرّق مقالاته الحقيقية بما لا تملكه المحاكاة (يوم المعلم، «حكاية
     أخبرني بها صديق»، دراسةٌ خاصة بالموضوع) — والمحرك لا يخترعها، فيأخذها منه. */
  const material = boundedString(value.material, 1_500)
  ...
  return { idea, audience, angle, material, targetWords, skipOriginality, styleProfile, styleSamples, existing, selectedEventIds, styleDna, variation, voiceExclusions }
}
...
export function domainKnowledge(idea, { angle = '', books = 4, articles = 2, interviews = 2, citations = 2, globals = 2, exclude = [], mustMatch = '' } = {}) {
  ...
  for (const document of documents) {
    ...
    if (anchorTerms.length && !anchorTerms.some((term) => document.tf.has(term))) continue
    let matched = 0
    let score = 0
...
/* ٢٥ سبتمبر ٢٠٢٦ — الخطط تصف الحركة ولا تسلّم ألفاظها: حَكَمٌ أعمى رصد في المحاكاة
   «ليست المشكلة… بل» مفتتحاً، و«المفارقة التي نتجاهلها»، و«المعيار الإنساني الذي
   نغفل عنه»، و«البديل…»، وعاميةً مصطنعة بين «…» — وكلها كانت ألفاظ هذه الخطط نفسها
   («افتح بنفيٍ… «ليست المشكلة في…بل في…»»، «ثم اكشف المفارقة… ثم ضع المعيار»،
   «(وقد تكون كويتيةً)»). وأُضيفت الأطروحة المكثّفة: أكثر مطالعه في آخر عشرين مقالاً
   مع المشهد، وكانت غائبة. وسُحب الافتتاح بالدراسة: صفرٌ من مطالعه العشرين الأخيرة
   (يستشهد في المتن). كل بنيةٍ تحمل حركة مطلعها (`move`) فتُختار بنسبه هو. */
const ARTICLE_FAMILIES = [
  {
    id: 'thesis',
    move: 'thesis',
    label: 'الأطروحة المكثّفة',
    plan: 'افتح بجملةٍ واحدة تحمل أطروحة المقال كلها كأنها عنوانه: قصيرة، تقرّر ولا تشرح. ثم افتح ما انطوت عليه فقرةً بعد فقرة: ما نراه كل يوم، ثم دلالته علينا، ثم ما يسنده من «من_مراجعك» أو من «من_عندك» إن وُجد. واختم بما حدّده الختام أدناه، بجملةٍ جديدة لا تعيد جملة المطلع ولا كلماتها.',
  },
  {
    id: 'scene',
    move: 'scene',
    label: 'المشهد اليومي',
    plan: 'افتح بجملةٍ قصيرة بالمضارع ترسم لحظةً عامّة يعرفها القارئ الكويتي في يومه، تختارها أنت من الفكرة نفسها، بلا بطلٍ مسمّى ولا «أنا» ولا حوار، ولا يتجاوز المشهد فقرته الأولى. ثم انتقل منها إلى الفكرة التي تحملها، بلفظك أنت لا بعبارة انتقالٍ جاهزة. ثم اسنده بما في «من_مراجعك» أو «من_عندك» إن وُجد. واختم بما حدّده الختام أدناه.',
  },
  {
    id: 'we',
    move: 'we',
    label: 'الضمير الجمعي',
    plan: 'افتح بجملةٍ بصوت «نحن» عن عادةٍ نمارسها دون أن ننتبه لثمنها. ثم بيّن ثمنها علينا كما تقتضيه الفكرة نفسها. ثم اسندها بما في «من_مراجعك» أو «من_عندك» إن وُجد. واختم بما حدّده الختام أدناه، لا بنصيحةٍ عملية.',
  },
  {
    id: 'negation',
    move: 'negation',
    label: 'النفي المزدوج',
    plan: 'افتح بجملة نفيٍ قصيرة تُسقط افتراضاً شائعاً عند القارئ، تصوغها أنت من الفكرة نفسها؛ ولا تستعمل الصيغة الجاهزة «ليست المشكلة في… بل في…». ثم فكّك الفهم الشائع بحجةٍ لا بحكاية. ثم ضع مكانه الفهم الأصدق بلفظك أنت، مسنوداً بما في «من_مراجعك» أو «من_عندك» إن وُجد. واختم بما حدّده الختام أدناه.',
  },
  {
    id: 'question',
    move: 'question',
    label: 'السؤال المعلّق',
    plan: 'افتح بسؤالٍ قصير يطرحه الواقع نفسه. ثم ناقش الجواب السهل الذي يريحنا وبيّن لماذا لا يكفي، في فقرةٍ واحدة لا في قائمة اعتراضات. ثم قدّم فهماً أعمق مسنوداً بما في «من_مراجعك» أو «من_عندك» إن وُجد. واختم بما حدّده الختام أدناه.',
  },
  {
    id: 'quote',
    move: 'quote',
    label: 'الكلمة الشائعة',
    plan: 'افتح بكلمةٍ أو كلمتين نقولهما كل يوم، بين «…» وبالفصحى (لا جملةٌ عامية ولا حوار). ثم افحص ما تكشفه هذه الكلمات عنّا. ثم بيّن ما تقوله عنّا بحجةٍ لا بحكاية. واختم بما حدّده الختام أدناه.',
  },
]

/* نسب مطالعه إن لم تصل البصمة بها: آخر عشرين مقالاً له حتى أبريل ٢٠٢٦. */
const FALLBACK_OPENING_SHARES = Object.freeze({ thesis: .25, scene: .3, we: .2, negation: .15, question: .05, quote: .05 })
...
/* بصمة الفكرة متعددة الحدود: فكرتان تختلفان في حرفٍ تتجاوران في باقي القسمة. الخلط
   (fmix32) يوزّعهما على المدى كله فتصدق النسب على عشرات الأفكار لا آلافها. */
const mixHash = (value) => {
  let hash = value >>> 0
...
/* عائلتان مختلفتان لكل طلب (خياران للدكتور لا نسختان)، تُختار كلٌّ منهما بنسب مطالعه
   هو: كانت تُختار بالتساوي، فافتتح «النفي» ثلاثة مقالاتٍ من عشرة وهو يفتتح به واحداً
   أو اثنين، وغابت الأطروحة المكثّفة. البصمة نفسها ورقم الجولة يحدّدان الاختيار. */
export function chooseFamilies(seedText, variation = 0, shares = null) {
  const weights = { ...FALLBACK_OPENING_SHARES, ...(shares && typeof shares === 'object' ? shares : {}) }
  const pick = (pool, salt) => {
    const total = pool.reduce((sum, family) => sum + Math.max(0, Number(weights[family.move]) || 0), 0)
    if (total <= 0) return pool[(familyFingerprint(`${seedText}|${salt}`) + variation) % pool.length]
    let roll = (mixHash(familyFingerprint(`${seedText}|${salt}`) + (Number(variation) || 0) * 7919) % 10_000) / 10_000 * total
    for (const family of pool) {
      roll -= Math.max(0, Number(weights[family.move]) || 0)
      if (roll < 0) return family
    }
    return pool[pool.length - 1]
  }
  const first = pick(ARTICLE_FAMILIES, 'first')
  const second = pick(ARTICLE_FAMILIES.filter((family) => family !== first), 'second')
  return [first, second]
}
...
export async function generatePerfectArticle(input, fetchImpl = fetch) {
  ...
  const citationCountRule = perArticle?.p50 >= 2
    ? `وحين يستشهد يسمّي في المقال نحو ${perArticle.p50} مراجع (بين 1 و${Math.max(perArticle.p50, perArticle.p85)})، موزّعةً على فقرات الحجة لا مرصوصةً في فقرةٍ واحدة؛ فاستشهد بكل ما يخدم فكرتك منها ومن «من_مراجعك» معاً، ولا تُدخل مرجعاً لا يخدمها لتبلغ العدد.`
    : 'مرجعٌ أو اثنان في المقال كله من «من_مراجعك» و«من_مراجع_العالم» معاً، لا أكثر.'
  const contentRules = [
    'قواعد المضمون:',
    ...
    hasGlobal ? `· «من_مراجع_العالم» داخل «معرفتك»: دراساتٌ وتقارير عالمية من مجلاتٍ محكّمة ومنظماتٍ دولية، تحقّقنا من كلٍّ منها من صفحته الأصلية. استشهد بمرجعٍ منها إن خدم فكرتك تحديداً: باسمه وسنته كما في «مرجع»، وبنتيجته كما في «ما_وجدته» بأرقامها نفسها وحذرها نفسه، فلا تجعل ما وجدته ارتباطاً سبباً، بصياغتك أنت. لا تزد عليها رقماً أو تعميماً ولا تنسب إليها ما لم تقله. ${citationCountRule}` : '',
  ...
  const closingFor = (family) => {
    ...
    const rollPerhaps = mixHash(familyFingerprint(`${input.idea}|${family.id}|perhaps`) + (Number(input.variation) || 0) * 7_919) % 100
    const rollLast = mixHash(familyFingerprint(`${input.idea}|${family.id}|last`) + (Number(input.variation) || 0) * 104_723) % 100
    /* ٢٦ سبتمبر ٢٠٢٦ (جولة M) — منطقة الختام لا الجملة الأخيرة: «فاسأل نفسك» في آخر ثلاث فقراتٍ من ثمانيةٍ من آخر
       عشرين مقالاً له، تليها غالباً فقرةٌ قصيرة تختم (١٠–١٩ كلمة)؛ و«وربما يبدأ» في ستة، وتجتمعان في أربعة. كانت
       النسبتان تُقاسان في الفقرة الأخيرة وحدها (٣ و٢ من ٢٠) فغابت «فاسأل نفسك» من عشر مسوداتٍ من عشر في جولة M،
       وعدّها الحَكَم الأعمى أوضح توقيعاته الغائبة. سحبتان مستقلتان للأداتين، ثم نوع الجملة الأخيرة بنسبته في
       فقراته الأخيرة: «بل» ثم السؤال ثم التقريرية. */
    const antithesis = Math.round((recent.closingAntithesisShare ?? .3) * 100)
    const question = Math.round((recent.closingQuestionShare ?? .1) * 100)
    const devices = [
      roll < ask ? 'قبيل النهاية، في الفقرة الأخيرة أو التي قبلها: «فاسأل نفسك:» ثم سؤالٌ أو سؤالان موجّهان إلى القارئ نفسه' : '',
      rollPerhaps < perhaps ? 'وفي منطقة الختام جملةٌ تفتح أفقاً تبدأ «وربما يبدأ… يوم…»' : '',
    ].filter(Boolean)
    const last = rollLast < antithesis
      ? 'انقلابٌ واحد بـ«بل» يقلب الفكرة في جملةٍ أو جملتين، بلا «لسنا بحاجة»'
      : rollLast < antithesis + question
        ? 'سؤالٌ واحد مفتوح يترك الفكرة معلّقةً عند القارئ'
        : 'جملةٌ تقريرية مكثّفة تثبّت الفكرة بلفظٍ جديد لا يعيد جملة المطلع، بلا «بل» ولا سؤال'
    return devices.length
      ? `الختام في هذا المقال: ${devices.join('؛ ')}؛ ثم تختم بفقرةٍ قصيرة: ${last}.`
      : `الختام في هذا المقال: ${last}. لا تستعمل فيه «فاسأل نفسك» ولا «وربما يبدأ».`
  }
  ...
  const payload = (family) => ({
    ...
    currentEvents,
    ...(input.material ? { من_عندك: input.material } : {}),
  })
  ...
  const evaluate = (draft) => {
    const verdict = judgeStyle(draft.body, dna, {
      ...
      generated: true,
      /* إلا ما جاء من مادته هو: حكايته حكايته، وجملته جملته. */
      authorMaterial: input.material || '',
  ...
  const families = chooseFamilies(`${input.idea} ${input.angle}`, input.variation, dna.recent?.openingShares)
  ...
  const firstRound = await Promise.allSettled(
    alwaysTwo
      ? [write(families[0], .62, primaryModel), write(families[1], .78, secondaryModel)]
  ...
  const citationSuggestions = (draft) => {
    ...
    const found = domainKnowledge(`${draft.title || ''} ${draft.excerpt || ''} ${draft.angle || ''}`, { exclude: exemplarSlugs, books: 0, articles: 0, interviews: 0, citations: 2, globals: 2, mustMatch: `${input.idea} ${input.angle || ''}` })
    const offered = [
  ...
  for (let round = 1; round <= maxRounds && canAfford(15_000); round += 1) {
    ...
    if (best.lengthOff > wordTolerance * 2 && canAfford(14_000)) {
  ...
  let proofread = null
  if (process.env.ARTICLE_PROOFREAD !== 'off' && canAfford(13_000)) {
