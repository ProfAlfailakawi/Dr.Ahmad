...
const MALE_MODE = process.argv.includes('--male-bakeoff')
const PAIRS_FEMALE = [
  /* الجولة الثالثة (٢٠ أغسطس ٢٠٢٦) — بحكمه على الثانية، وهو حكمٌ التقى فيه
     مع القياس لأول مرة: مدح Puck وحده ومدح Autonoe وحدها، ثم قال عن
     اجتماعهما «غير مناسب». وقياسهما ٢ هرتزاً. فما رفضه ليس صوتاً بل تشابهاً.

     Puck ثابتٌ هنا لأنه الصوت الوحيد الذي مدحه في ثلاث جولات («جيد»،
     «جيد جدا»). والمتغيّر امرأةٌ تقع فوقه بوضوح: كل من جُرّبن حتى الآن
     يقعن في طبقته نفسها (Autonoe ١٧٨–١٩٨ · Aoede ١٧٢ · Erinome ١٨٢ ·
     Leda ١٨٦) أو تحتها (Callirrhoe ١٦٠–١٧٦). المطلوب وسيطٌ فوق ٢١٠. */
  /* ═══ [٢٢ أغسطس ٢٠٢٦] انقلاب الاتجاه ═══
     كل الجولات السابقة ثبّتت Puck وبحثت عن امرأةٍ فوق ٢١٠ — ولم تُوجد
     واحدة في إحدى عشرة محاولة. والاتجاه المعاكس لم يُجرَّب قط: **رجلٌ
     أعمق من Puck**. وصار مُلحّاً بعد كناريا v3: المداخلات القصيرة جداً
     (٨١٪ دون ٥٥ حرفاً) ترفع نبرة Puck من ~١٤٠ إلى ١٥٥–١٦٢، فيلتقي
     بـZephyr وتنهار الفجوة (١١ · ١١ · ‑٢ هرتز في ثلاث عيّنات متتالية).
     فالمتغيّر الآن الرجل، وZephyr ثابتة لأن الدكتور اختارها بأذنه («روعه»).
     شغّله بـ--male-bakeoff. */
  ['واحد', 'Puck', 'Kore',          'مرشّحة عالية'],
  ['اثنين', 'Puck', 'Zephyr',        'مرشّحة عالية'],
  ['ثلاثة', 'Puck', 'Achernar',      'مرشّحة عالية'],
  ['أربعة', 'Puck', 'Sulafat',       'مرشّحة عالية'],
  ['خمسة', 'Puck', 'Laomedeia',     'مرشّحة عالية'],
  ['ستة',  'Puck', 'Autonoe',       'مرجع — رفضتَه لتشابه الطبقة'],
]

/* مرشّحو الرجل الأعمق — Zephyr ثابتة (اختياره سماعاً) */
const MALE_PAIRS = [
  ['واحد', 'Charon',        'Zephyr', 'رجل أعمق — مرشّح أول'],
  ['اثنين', 'Fenrir',       'Zephyr', 'رجل أعمق'],
  ['ثلاثة', 'Orus',         'Zephyr', 'رجل أعمق'],
  ['أربعة', 'Enceladus',    'Zephyr', 'رجل أعمق'],
  ['خمسة', 'Iapetus',       'Zephyr', 'رجل أعمق'],
  ['ستة',  'Puck',          'Zephyr', 'مرجع — الزوج الحالي للمقارنة'],
]
