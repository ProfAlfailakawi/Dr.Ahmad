...
assert.match(portal, /open=\{isOpen\}/)
assert.match(portal, /setOpenDoorId\(\(current\) => current === door\.id \? '' : door\.id\)/)
assert.doesNotMatch(portal, /detailsRef\.current\.open = true/)
