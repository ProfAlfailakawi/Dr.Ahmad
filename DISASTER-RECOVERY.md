...
|---|---|---|
| Firestore PITR | **معطّل** | لا استرجاع لنقطة زمنية؛ أقصى ما يُفقد = منذ آخر نسخة يومية |
| Firestore حماية الحذف | **معطّلة** | قاعدة البيانات كلها قابلة للحذف بأمر واحد |
| Firestore نسخ مُدارة | **صفر جدول** | لا نسخ مُدارة من Google |
| Firebase Storage versioning | **مُعطّل (Suspended)** | الكتابة فوق ملف أو حذفه لا رجعة فيه |
| Storage lifecycle | لا شيء | — |
| حجم Storage كامل | **~٧١ ميغابايت** | صغير، مرآته سهلة ورخيصة/مجانية |
| Cloudflare R2 versioning | **لا شيء** | حذف/دهس كائن صوت لا رجعة فيه |
...
4. **لا نسخ لـFirebase Storage ولا لصوت R2 إطلاقاً** — ما زالت مفتوحة (أوامر المالك أدناه).
