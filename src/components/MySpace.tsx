...
import { ASK_LIBRARY_MEMORY_EVENT, askLibraryMemories } from "../lib/ask-library-memory";
import {
  SPACE_EVENT,
  isArticleSaved,
  isMediaSaved,
            ...
            <>
              <div className="rounded-xl border border-hair bg-canvas p-3">
                ...
                <div className="mt-2 flex items-center justify-between gap-3">
                  ...
                  <button type="button" onClick={copyCode} aria-label="نسخ رمز المزامنة" title="نسخ الرمز" className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-hair text-soft hover:border-accent hover:text-accent"><SocialIcon name="Copy" size={15} /></button>
                </div>
              </div>
...
export function MySpace({ variant = "floating" }: { variant?: "floating" | "footer" }) {
  ...
  const askSessions = useMemo(() => askLibraryMemories(), [version]);
  ...
  const hasJourney = Boolean(snapshot.last || snapshot.audio || snapshot.saved.length || snapshot.savedMedia.length || snapshot.recent.length || snapshot.quotes.length || askSessions.length);
            ...
            <motion.div className="my-space-overlay fixed inset-0 z-[310] flex items-end justify-center bg-ink/25 px-3 pb-[calc(.75rem+env(safe-area-inset-bottom))] backdrop-blur-[3px] md:items-center md:p-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: .2 }} onMouseDown={(event) => { if (event.target === event.currentTarget) closeSpace(); }} role="presentation">
              <motion.section ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="my-space-title" initial={{ opacity: 0, y: 28, scale: .985 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 18, scale: .99 }} transition={{ duration: .38, ease: EASE }} className="my-space-panel relative flex max-h-[min(780px,90dvh)] w-full max-w-[780px] flex-col overflow-hidden rounded-[1.75rem] border border-hair bg-canvas shadow-[0_40px_120px_-44px_rgba(15,25,36,.7)]">
                <header className="my-space-head shrink-0 border-b border-hair bg-canvas/[.95] px-5 pt-5 backdrop-blur md:px-8 md:pt-6">
                  ...
                  <nav className="edge-fade mt-5 flex gap-6 overflow-x-auto" role="tablist" aria-label="أقسام مساحتي">
                    {tabs.map((item) => (
                        ...
                        <div className="space-y-7">
                          ...
                          {otherAskSessions.length > 0 && (
                            <div className="grid gap-3 sm:grid-cols-2">
                              {otherAskSessions.map((session) => <Link key={session.id} to={`/ask?q=${encodeURIComponent(session.question)}`} onClick={() => setOpen(false)} className="ask-memory-card group min-w-0 rounded-2xl border border-hair bg-canvas p-4 transition hover:border-accent"><div className="flex items-start justify-between gap-3"><strong className="line-clamp-2 min-w-0 font-display text-[.88rem] leading-[1.65] text-ink group-hover:text-accent">{session.question}</strong><span className="shrink-0 text-[.62rem] text-soft">{timeAgo(session.updatedAt)}</span></div><span className="mt-2 line-clamp-2 block text-[.7rem] font-light leading-[1.75] text-soft">{session.summary}</span>{session.branches.length > 0 && <span className="mt-3 flex flex-wrap gap-1">{session.branches.slice(0, 2).map((branch) => <span key={branch.id} className="max-w-full truncate rounded-full border border-accent/[.15] px-2 py-1 text-[.58rem] text-soft">{branch.title}</span>)}</span>}</Link>)}
                            </div>
                          )}
