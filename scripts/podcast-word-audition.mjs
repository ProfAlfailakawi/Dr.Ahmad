...
/* كل كلمةٍ شكا منها الدكتور (٢٠ أغسطس ٢٠٢٦) مع جملتها الحاملة من المتن.
   ...
   لا كلمة مخترعة هنا. */
const WORDS = [
  /* الجولة الخامسة (٢٠ أغسطس ٢٠٢٦) — وهي أهمّ جولةٍ حتى الآن، لأن الدكتور
     أعطى **تشخيصاً** لا قائمة كلمات: «قاعد يحط سكون وهذا خطأ» و«فيها سكون
     علشان جذي بدلها» و«دمجهم قاعد يحولهم اماراتي».

     أي أن الانحراف الإماراتي ليس في اختيار الكلمة بل في **إسكان الحركات**:
     المحرّك يبتلع الحركة القصيرة فيلتصق الساكنان، وهذا بالضبط إيقاع الإمارات.
     والكويتية الحضرية تُبقي الحركة خفيفةً ممتلئة.

     فهذي الجولة تختبر **الفرضية** لا الكلمات: هل التشكيل الصريح يمنع
     الإسكان؟ لكل كلمةٍ ثلاثة: كما هي · مشكَّلةً بالحركات · بديلٌ آخر.
     إن نجح التشكيل في الأربع فهي قاعدةٌ نطبّقها على المعجم كله لا خمس كلمات. */
  { key: 'هو — يُسكِّنها', speaker: 'Fahad',
    carrier: 'ترى يمكن {W} اللي يگلب الفكرة كلها.',
    options: ['هو', 'هُوَ', 'هوَّ'] },
  { key: 'المشكلة — سكون', speaker: 'Noura',
    carrier: 'و{W} مو إننا نفرح بالنتيجة. بالعكس، نفرح.',
    options: ['المشكلة', 'المُشْكِلة', 'المُشكِلَة'] },
  { key: 'تلخص — إماراتية', speaker: 'Noura',
    carrier: 'صج، هذي كلمة {W} إحساس جيل كامل.',
    options: ['تلخص', 'تُلَخِّص', 'تجمع'] },
  { key: 'ضعف ثقته — إماراتية', speaker: 'Fahad',
    carrier: 'هالشي يجره بعدين ل{W} بنفسه.',
    options: ['ضعف ثقته', 'ضَعْف ثِقْته', 'قلة ثقته'] },
]
