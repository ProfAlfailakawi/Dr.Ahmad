...
const ARTICLE_FAMILIES = [
  ...
  },
  {
    ...
    move: 'scene',
    label: 'المشهد اليومي',
    plan: 'افتح بجملةٍ قصيرة بالمضارع ترسم لحظةً عامّة يعرفها كل بيتٍ أو صفٍّ كويتي، تختارها أنت من الفكرة نفسها، بلا بطلٍ مسمّى ولا «أنا» ولا حوار، ولا يتجاوز المشهد فقرته الأولى. ثم انتقل إلى ما لا يظهر في المشهد نفسه. ثم اسنده بما في «من_مراجعك» أو «من_عندك» إن وُجد. واختم بما حدّده الختام أدناه.',
  },
  {
    id: 'we',
    move: 'we',
    label: 'الضمير الجمعي',
    plan: 'افتح بجملةٍ بصوت «نحن» عن عادةٍ نمارسها دون أن ننتبه لثمنها. ثم اكشف ما تخفيه هذه العادة في أبنائنا ومدارسنا. ثم اسندها بما في «من_مراجعك» أو «من_عندك» إن وُجد. واختم بما حدّده الختام أدناه، لا بنصيحةٍ عملية.',
  },
  {
    id: 'negation',
    move: 'negation',
  ...
  },
  {
    ...
    plan: 'افتح بكلمةٍ أو كلمتين نقولهما كل يوم، بين «…» وبالفصحى (لا جملةٌ عامية ولا حوار). ثم افحص ما تكشفه هذه الكلمات عنّا. ثم اقرأ ما وراءها تربوياً بحجةٍ لا بحكاية. واختم بما حدّده الختام أدناه.',
  },
]
...
/* ٢٦ سبتمبر ٢٠٢٦ — جولة J: سحبت المسوداتُ الأفكارَ العامة إلى المدرسة. آخر عشرين مقالاً له: ستةٌ بلا فقرةٍ
   مدرسية واحدة، وفي أفكاره العامة («الجدية قناعاً للهروب»، «العودة…»، «السنة…») صفرٌ من الفقرات، بينما
   جعلت المسودات فيها ٣٠٪ و٧٥٪ و٦٧٪، وعدّه الحَكَم الأعمى «سحباً قسرياً إلى الصف». الفكرة مدرسيةٌ إن
   حملت مفرداتها؛ و«التقويم» ليست منها عمداً (تقويم السنة). */
const SCHOOL_WORDS = /(?<!\p{L})(?:و|ف|ب|ل|لل|ال|وال|بال|فال)*(?:مدرس|مدارس|مدرسي|صف|صفوف|معلم|معلمين|معلمون|طالب|طلاب|طلبه|تلاميذ|حصه|حصص|امتحان|امتحانات|اختبار|اختبارات|منهج|مناهج|قياد|قيادات|تعليم|تعلم|دراسي|جامعه|درس|دروس|واجب|واجبات)(?:ه|ها|هم|نا|ات|ين|ون|يه|ا)?(?!\p{L})/u
...
const GENERAL_IDEA_RULE = '· الفكرة لم تذكر المدرسة، فلا تفترض الصفّ إطاراً لها: ابدأ من الإنسان في حياته اليومية وعمله وبيته كما يكتب مقالاته العامة (ثلاثةٌ من أفكاره العامة الست في ٢٠٢٦ لا تذكر المدرسة أصلاً). وإن احتاجت الحجةُ المدرسةَ فلتكن جزءاً لا إطاراً: لا تتجاوز فقراتُ الصفّ والمعلّم والطالب والقيادة نصفَ المقال.'
...
export async function generatePerfectArticle(input, fetchImpl = fetch) {
  ...
  const closingFor = (family) => {
    ...
    /* ٢٦ سبتمبر ٢٠٢٦ — كل ختامٍ بنسبته المقيسة في فقراته الأخيرة: «فاسأل نفسك» ثم «ربما يبدأ» ثم «بل»
       ثم السؤال، والباقي جملةٌ تقريرية مكثّفة. كان الباقي كله «…بل» فختمت به ثماني مسوداتٍ من عشر. */
  ...
  /* الفكرة العامة التي صارت مقالاً مدرسياً: فقرات المدرسة في أفكاره العامة الست في ٢٠٢٦ صفرٌ في ثلاث
     و٣٨٪ و٤٣٪ و٧٥٪ (في واحدة)، وفي المسودات ٣٠٪ و٤٠٪ و٦٧٪ و٧٥٪ و٧٥٪ و٧٨٪. العتبة فوق ما بلغه في
     خمسٍ من ست، ودون أغلب المسودات. */
  ...
  for (let round = 1; round <= maxRounds && canAfford(15_000); round += 1) {
    ...
    if (drifted) {
      orders.push(`الفكرة لم تذكر المدرسة، و${Math.round(schoolParagraphShare(best.draft.body) * 100)}٪ من فقراتك تدور في الصف والمعلّم والطالب والقيادة. أعد أكثرها إلى الإنسان في حياته اليومية كما هي الفكرة، ولتبقَ المدرسة جزءاً لا إطاراً: نصف الفقرات على الأكثر.`)
    }
