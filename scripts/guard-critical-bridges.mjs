...
const assertions = [
  ...
  [whatsappBridge.includes('watchdog_restart_stuck_authenticated') && whatsappBridge.includes("process.exit(75)") && whatsappBridge.includes("WHATSAPP_BRIDGE_SECRET || ''") && whatsappWebBridge.includes('sendTextWithRecovery') && whatsappWebBridge.includes("'send-self-message'") && whatsappWebBridge.includes('waitUntilMsgSent: true') && whatsappWebBridge.includes('duplicate_command_acknowledged_without_resend') && whatsappController.includes("path === '/emergency-stop'"), 'WhatsApp bridge must self-restart when stuck, recover modern delivery, stop queued sends, deduplicate commands, and never ship a fallback secret'],
