...
const generationLibraryMarker = 'data-generation-library=\"true\"'
const generationLibraryIndex = studio.indexOf(generationLibraryMarker)
...
ok(generationLibraryIndex > studio.indexOf('<StageRail')
  && generationLibraryIndex < simplifiedIdeaIndex
  && generationLibraryIndex < fullIdeaIndex
  && studio.split(generationLibraryMarker).length === 2, 'مكتبة التوليد تُرندر مرة واحدة خارج فرعي الاستوديو المبسط والكامل وتظهر دائماً')
...
ok(studio.includes('backfillGenerationLibrary') && studio.includes('استعادة المحفوظات القديمة') && studio.includes('legacy-design-') && studio.includes('legacy-image-'), 'المكتبة تستعيد بأمان المحفوظات المحلية القديمة والأصول المضمّنة بمعرّفات ثابتة غير مكررة')
