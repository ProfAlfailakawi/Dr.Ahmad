...
/* كل كلمةٍ شكا منها الدكتور (٢٠ أغسطس ٢٠٢٦) مع جملتها الحاملة من المتن.
   ...
   لا كلمة مخترعة هنا. */
const WORDS = [
  /* ═══ الجولة الرابعة عشرة (٢٢ أغسطس ٢٠٢٦) — كلمتان من حكم النسخ الثلاث ═══ */

  /* حلقة ٢: «يعرفها بعقله مو راضي يضبطها — أتوقع نبدل الجملة كاملة».
     ثلاث محاولات على الكلمة سقطت (عارفها ثم داري فيها ثم يعرفها)،
     فهذه بدائل جملة كاملة كما أمر. */
  { key: 'يعرفها بعقله — جملة كاملة', speaker: 'Fahad',
    carrier: '{W}. بس الإحساس مو دايما يسمع كلام العقل.',
    options: ['يعرفها بعقله، إي', 'مخه مستوعبها، إي', 'يدري إنها صح، إي'] },

  /* حلقة ٣: «بباب ضيق — المفروض معدلينها من زمان». هي معدلة فعلاً
     («ظيّج» تصل المحرك) لكن العينة تتذبذب — وهذا التذبذب موثق في
     طبقة variance: نفس المدخل ممتاز في تشغيلة وسيئ في التالية.
     فنجرب تشكيلاً كاملاً يضيق على المحرك، وبديل مكان. */
  { key: 'بباب ظيّج', speaker: 'Fahad',
    carrier: 'بس راحة مؤقتة… مثل واحد كان محشور {W} وطِلَع مِنّه.',
    options: ['بباب ظيّج', 'بباب ظَيِّج', 'بمكان ظيّج'] },
]
