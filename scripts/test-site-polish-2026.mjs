...
ok(workflow.includes('Generation Library Storage unavailable') && workflow.includes('exit 1') && workflow.includes('Generation Library Storage rules deployed successfully'), 'نشر الموقع يتوقف إذا لم تُنشر قواعد Storage لمكتبة التوليد؛ لا نجاح زائف للمكتبة')
const storageHealth = read('scripts/verify-generation-library-storage.mjs')
ok(workflow.includes('node scripts/verify-generation-library-storage.mjs') && storageHealth.includes('file.save') && storageHealth.includes('file.download') && storageHealth.includes('file.delete'), 'CI يختبر حاوية Storage فعلياً كتابةً وقراءةً وحذفاً قبل نشر Hosting')
...
ok(!extras.includes("label: 'قراءة نورة'") && !extras.includes("label: 'صوت نورة'") && extras.includes("label: 'قراءة المقال'") && extras.includes('voices.noura'), 'صفحة المقال وحدها تخفي اسم نورة وتعرض المسار باسم «قراءة المقال» مع تفضيل ملف نورة عند وجوده')
