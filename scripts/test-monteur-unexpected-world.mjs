...
assert.match(source, /function signatureRecipeFor/, 'every topic receives a deterministic semantic signature')
assert.match(source, /data-signature/, 'the climax exposes its unique signature identity')
assert.match(source, /textComposition/, 'text composition changes across the story')
assert.match(source, /memorySealMarkup/, 'the ending remembers the climax signature')
assert.match(css, /\.impossible-world/, 'the impossible world has a production visual layer')
assert.match(css, /\.topic-signature/, 'the semantic signature has its own cinematic layer')
assert.match(css, /data-composition="revelation"/, 'the climax includes a silent visual reveal')
assert.match(css, /\.memory-seal/, 'the last frame keeps a memorable visual seal')
