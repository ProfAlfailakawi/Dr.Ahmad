...
/* كل كلمةٍ شكا منها الدكتور (٢٠ أغسطس ٢٠٢٦) مع جملتها الحاملة من المتن.
   ...
   لا كلمة مخترعة هنا. */
const WORDS = [
  /* ═══ الجولة العاشرة (٢١ أغسطس ٢٠٢٦ ليلاً — بعد أحكام التاسعة) ═══
     أربع بقايا فقط؛ الثمان الأخرى أُقفلت في المعجم بأرقامه.
     الجمل الحاملة صارت بإملاء المعجم الجديد (عقب لا بعدين، إسِله). */

  /* حكمه على الجولة التاسعة ٤: «تحتاج تبديل، مو مضبوط» — الهجاءات الثلاثة
     سقطت، فهذه بدائل استبدالٍ كاملة. «بروحه» كويتية «بنفسه». */
  { key: 'لظعف ثقته بنفسه — بدائل', speaker: 'Noura',
    carrier: 'لما الواحد يربط قيمته بروحه بدرجاته، هالشي {W}.',
    options: ['يجره عقب لظعف ثقته بنفسه', 'يخلي ثقته بروحه تظعف', 'يظيّع ثقته بروحه'] },

  /* «طالب ناجح» — قالتها نورة بطريقة غريبة (دور ٢٢). البديل الثالث
     استبدال كويتي طبيعي: «تشوف واحد ناجح». */
  { key: 'طالب', speaker: 'Noura',
    carrier: 'وهني تشوف {W} ناجح، بس مو فرحان.',
    options: ['طالب', 'طَالِب', 'واحد'] },

  /* «منو غلب» — أمره: «ممكن نبدلها بكلمة ثانية إذا دايم خطأ».
     «ربح» كويتية شائعة للفوز، و«منو الأول» من بدائل الجولة السابعة. */
  { key: 'منو غلب — بدائل', speaker: 'Noura',
    carrier: 'ومو: {W}؟ إسِله: انت اليوم أنظج شوي من نفسك أمس؟',
    options: ['منو غلب', 'منو ربح', 'منو الأول'] },

  /* «يي» — سمعها «يجي» («ما نقول يجي بالعامية») والنص يكتب «يي» أصلاً:
     المحرك يضيف الجيم من عنده. نجرب هجاءين يقفلانها، والثالث بهمزة
     الوصل على قاعدته «احنا دايما نحط همزة وصل». */
  { key: 'يي', speaker: 'Fahad',
    carrier: 'يصير نهاية توتر… لين {W} الاختبار اللي بعده.',
    options: ['يي', 'يِي', 'ايي'] },

  /* ═══ فحص المتن الكامل قبل الإطلاق (٢١ أغسطس ٢٠٢٦ ليلاً) ═══
     مسحُ ٥٥٨٠ مداخلةً في الحلقات الـ١٤٤ بأنماط فشل الشهر أخرج هذه
     الثماني — الأعلى تكراراً × الأشبه بما سقط سابقاً. الجمل الحاملة
     حقيقيةٌ من المتن وبإملاء مدخل الصوت، والأرقام أمام كل كلمةٍ هي
     عدد مواضعها في المتن كله. */

  /* بالمئة — ٥٢ موضعاً. الهمزة الوسطية تجر للفصحى، والكويتي «بالمية». */
  { key: 'بالمئة', speaker: 'Noura',
    carrier: 'وإن قريب من خمسين {W} ما يعرفون يوصفون أنفسهم.',
    options: ['بالمئة', 'بالمية', 'في المية'] },

  /* السؤال — ١٠٦ مواضع لعائلة سؤال. نفس علة «اسأله» المحسومة (إسِله). */
  { key: 'السؤال', speaker: 'Fahad',
    carrier: 'ترى كان أكو زمن… الشهادة تنطبع فيه قبل ما يوصل {W} أصلا.',
    options: ['السؤال', 'السوال', 'السُوال'] },

  /* يسأل — ٢٩ موضعاً. */
  { key: 'يسأل', speaker: 'Fahad',
    carrier: 'المقال ما يلومه أبدا. هُوَ {W} عن النظام نفسه.',
    options: ['يسأل', 'يسال', 'يِسال'] },

  /* قدام — ٩٢ موضعاً. موروثة، والحضري «جدام» (القاف جيماً في الموروث). */
  { key: 'قدامك', speaker: 'Noura',
    carrier: 'المعلومة اليوم ساكنة عند طرف الإبهام… لمسة وحدة وتحظر {W}.',
    options: ['قدامك', 'جدامك', 'قِدّامك'] },

  /* هذا — ٢٦٤ موضعاً، أخت «هاذي» التي أملاها. و«جذي» بديل كويتي قح
     لسياق «ومع هذا». */
  { key: 'هذا', speaker: 'Fahad',
    carrier: 'وهالدولة ما عطلت مدارسها بعد… ومع {W} حطت الخطة البديلة.',
    options: ['هذا', 'هاذا', 'جذي'] },

  /* هذولا — ٣٣ موضعاً، والمتن نفسه متردد بين هذولا وهذيلا. */
  { key: 'هذولا', speaker: 'Noura',
    carrier: 'و{W} هم الأبطال صِج.',
    options: ['هذولا', 'هذيلا', 'هاذيلا'] },

  /* طلع بمعنى خرج — عائلة طلع ١٠٩ مواضع، وسبق أن سقطت «طلع منه»
     وعلاجها التشكيل (طِلَع مِنّه بأذنه). */
  { key: 'طلع — خرج', speaker: 'Fahad',
    carrier: 'جم طفل {W} من المدرسة وظهره مستقيم… ومخه مكسور؟',
    options: ['طلع', 'طِلَع', 'طَلَع'] },

  /* طلع بمعنى تبيّن («طلع إن») — معنى ثانٍ لنفس الكلمة، يُختبر مستقلاً
     على قاعدة «لا يُعمَّم بديل على كلمة لها أكثر من معنى». */
  { key: 'طلع إن — تبيّن', speaker: 'Noura',
    carrier: 'بدراسة نشرتها جامعة هارفارد، {W} إن العيال يبينون ظعف بالاستقلالية.',
    options: ['طلع', 'طِلَع', 'تبين'] },
]
