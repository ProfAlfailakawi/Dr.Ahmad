        ...
        {related.length > 0 && <FadeUp delay={.2}><section className="mt-12 border-t border-hair pt-8" aria-labelledby="media-related-title">
          ...
          <div dir="rtl" className="media-related-rail mt-5 flex snap-x snap-mandatory gap-4 overflow-x-auto pb-3 [touch-action:pan-x]">
            {related.map((entry) => <Link key={entry.slug} to={`/media/${entry.slug}`} className="w-[min(82vw,19rem)] shrink-0 snap-start overflow-hidden rounded-2xl border border-hair bg-canvas transition hover:border-accent">
              <div className={`aspect-video overflow-hidden bg-wash ${entry.id && entry.kind !== 'audio' && entry.kind !== 'radio' ? 'complete-media-frame' : ''}`} style={entry.id && entry.kind !== 'audio' && entry.kind !== 'radio' ? ({ '--media-thumb': `url(${entry.thumbnail || `https://i.ytimg.com/vi/${entry.id}/hqdefault.jpg`})` } as CSSProperties) : undefined}>{entry.id && entry.kind !== 'audio' && entry.kind !== 'radio' ? <img src={entry.thumbnail || `https://i.ytimg.com/vi/${entry.id}/hqdefault.jpg`} alt="" loading="lazy" onLoad={(event) => { const img = event.currentTarget; if (!entry.thumbnail && img.naturalWidth <= 120 && img.src.includes('/hqdefault.')) img.src = `https://i.ytimg.com/vi/${entry.id}/mqdefault.jpg`; }} onError={(event) => { const img = event.currentTarget; if (img.src.includes('/hqdefault.')) img.src = `https://i.ytimg.com/vi/${entry.id}/mqdefault.jpg`; else img.style.display = 'none'; }} className="complete-media-image h-full w-full" /> : <div className="flex h-full items-center justify-center text-accent"><span className="flex h-14 w-14 items-center justify-center rounded-full border border-accent/[.22] bg-canvas/[.72]"><SocialIcon name="Play" size={17} /></span></div>}</div>
