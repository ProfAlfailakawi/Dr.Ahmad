    ...
    <details data-generation-library="true" className="mt-4 overflow-hidden rounded-2xl border border-accent/20 bg-accent/[.025]">
      ...
      <div className="grid gap-5 border-t border-hair p-3 md:p-4">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-hair bg-paper/70 px-3 py-2.5">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <span className={`rounded-full px-2.5 py-1 text-[.75rem] font-bold ${storageStatus === 'ready' ? 'bg-emerald-100 text-emerald-700' : storageStatus === 'failed' ? 'bg-amber-100 text-amber-800' : 'bg-wash text-soft'}`}>{storageStatus === 'ready' ? 'Storage جاهز' : storageStatus === 'checking' ? 'أفحص…' : storageStatus === 'failed' ? 'يحتاج مراجعة' : 'غير مفحوص'}</span>
        ...
        {backfillSummary && <p className="rounded-xl border border-accent/[.15] bg-accent/[.035] px-3 py-2 text-[.75rem] leading-relaxed text-soft">{backfillSummary}</p>}
          ...
          {designBusy && !designs.length ? <p className="rounded-xl border border-dashed border-hair px-4 py-5 text-[.68rem] text-soft">أحمّل آخر تصميم معتمد…</p> : filteredDesigns.length ? (
            <div className="mobile-card-rail flex snap-x snap-mandatory gap-3 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              {filteredDesigns.map((asset) => <article key={asset.id} className="relative w-[210px] shrink-0 snap-start overflow-hidden rounded-2xl border border-hair bg-canvas shadow-[0_12px_32px_rgba(17,41,75,.045)]">
                ...
                {asset.thumbnail ? <img src={asset.thumbnail} alt="" className="aspect-[4/3] w-full object-cover" loading="lazy" /> : <div className="relative grid aspect-[4/3] w-full place-items-center overflow-hidden bg-[radial-gradient(circle_at_22%_18%,rgba(47,111,142,.16),transparent_34%),linear-gradient(145deg,var(--paper),var(--canvas))]"><span className="absolute inset-x-5 top-5 h-px bg-accent/[.15]" /><span className="font-display text-3xl font-black text-accent/25">A</span><span className="absolute bottom-4 end-4 text-[.75rem] font-bold tracking-[.18em] text-soft/[.65]">ARCHIVE</span></div>}
                <div className="grid gap-2 p-3 text-right"><div className="flex items-center justify-between gap-2"><span className="rounded-full border border-accent/[.15] bg-accent/[.045] px-2 py-1 text-[.75rem] font-semibold text-accent">{asset.generationKind}</span>{asset.quality > 0 && <span className="text-[.75rem] font-semibold text-soft">{Math.round(asset.quality)}٪</span>}</div><strong className="line-clamp-2 min-h-[2.5rem] text-[.75rem] leading-relaxed text-ink">{asset.title}</strong><span className="line-clamp-1 text-[.75rem] text-soft">{asset.visualWorld || asset.directionLabel || asset.note || 'نسخة محفوظة'}</span><span className="text-[.75rem] text-soft" dir="ltr">{asset.width}×{asset.height}{asset.formatLabel ? ` · ${asset.formatLabel}` : ''} · {asset.createdAtMs ? new Date(asset.createdAtMs).toLocaleDateString('ar-KW-u-nu-latn') : ''}</span><div className="flex gap-2"><button type="button" className={`${primaryClass} flex-1 px-3 py-2 text-[.62rem]`} disabled={designBusy} onClick={() => onUseDesign(asset)}>فتح للتعديل</button><button type="button" className={`${ghostClass} px-3 py-2 text-[.62rem]`} disabled={designBusy} onClick={() => onDeleteDesign(asset)}>حذف</button></div></div>
              </article>)}
            </div>
