...
function SyncedArticleBody({ slug, body, title }: { slug: string; body: string; title: string }) {
  ...
  const readerMade = useMemo(() => {
    ...
    return [...popularQuotes]
      .sort((left, right) => right.count - left.count || left.paragraph - right.paragraph || left.startOffset - right.startOffset)
      .map((quote) => ({
        ...quote,
        text: (sourceParagraphs[quote.paragraph] || '').slice(quote.startOffset, quote.endOffset).trim(),
      }))
      .filter((quote) => quote.text.length >= 12)
  }, [body, popularQuotes])
  const readerMadePages = usePagedList(readerMade, 6, `${slug}:${readerMade.map((item) => item.highlightKey).join('|')}`)
        ...
        <section className="reader-made-archive mt-12 border-t border-hair pt-8" aria-labelledby="reader-made-title">
          <details className="group">
            ...
            <div id="reader-made-highlights" className="mt-4 scroll-mt-28 rounded-2xl border border-hair bg-canvas p-4 md:p-6">
              ...
              <Pagination
                page={readerMadePages.page}
                pageCount={readerMadePages.pageCount}
                onChange={readerMadePages.setPage}
                totalItems={readerMade.length}
                firstItem={readerMadePages.firstItem}
