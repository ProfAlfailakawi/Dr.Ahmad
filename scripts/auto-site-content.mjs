...
function sourceSentences(source) {
  return clean(source?.text)
    .replace(/<[^>]+>/g, ' ')
    .split(/(?<=[.!؟…])\s+/)
    .map((sentence) => sentence.replace(/^[\s«»"'-]+|[\s«»"']+$/g, '').replace(/[،؛:.!؟…]+$/g, '').trim())
...
const rotate = (list, salt) => list[Number.parseInt(hash(salt), 16) % list.length]
...
const hasProse = (source) => source?.type === 'مقال'
  || (source?.type === 'كتاب' && clean(source?.text).length >= 120)

function pickSentence(source, salt) {
  if (!hasProse(source)) return ''
  const all = sourceSentences(source)
  if (!all.length) return ''
  /* نُفضّل الجملة المتوسطة الطول: القصيرة جداً لا تحمل فكرة، والطويلة تُثقل */
  const ranked = [...all].sort((a, b) => Math.abs(a.length - 150) - Math.abs(b.length - 150))
  const pool = ranked.slice(0, Math.min(6, ranked.length))
  return rotate(pool, `${source.key}:${salt}`)
}

function compactSourceExcerpt(source, limit = 235) {
  const sentence = pickSentence(source, 'excerpt')
  if (!sentence) return 'أن قيمة الفكرة لا تظهر في جمالها النظري فقط، بل في القرار أو الممارسة التي تغيّرها.'
  const excerpt = sentence.slice(0, limit).trim()
  return excerpt.length < sentence.length ? `${excerpt.replace(/[،؛:.!?؟\s]+$/g, '')}…` : excerpt
}

const SOURCE_WORD = { 'مقال': 'مقالك', 'كتاب': 'كتابك', 'لقاء': 'لقاءك' }

const LETTER_OPENERS = [
  (s) => `توقفت طويلاً عند ${SOURCE_WORD[s.type] || 'مادتك'} «${s.title}»، وبقيت عندي جملةٌ منه أعيد قراءتها:`,
  (s) => `قرأت «${s.title}» على مهل، ولم أخرج منه بانطباعٍ واحد بل بجملةٍ ظلّت معي:`,
  (s) => `في «${s.title}» عبارةٌ لخّصت ما كنت أحاول قوله منذ مدة ولا أُحسن صياغته:`,
  (s) => `عدت إلى «${s.title}» مرةً ثانية، والسبب سطرٌ فيه لم يفارقني:`,
  (s) => `ما يميّز «${s.title}» أنه لا يكتفي بوصف ${s.category || 'المسألة'}؛ ففيه هذا القول:`,
...
/* ═══ الأسئلة — من مجالات الدكتور وتخصصه ═══
 *
 * السؤال إطارٌ لا دعوى، فلا حرج أن يُصاغ عندنا. أما الجواب فدعوى تُنسب إليه،
 * فيُؤخذ من متنه هو. وهذا هو الفرق الذي كان غائباً: القالب القديم كان يخترع
 * السؤال والجواب معاً، ويكتبهما بالصيغة نفسها في كل مرة.
 */
const FAQ_FRAMES = [
...
function fallbackFaq(source, topic) {
  ...
  /* الجواب من كلام الدكتور نفسه. وإن لم يوجد في المتن جملةٌ صالحة، رجعنا إلى
     معياره الثابت المكتوب هنا — وهو كلامُ ربطٍ لا دعوى على مادةٍ بعينها. */
  const sentence = pickSentence(source, `faq:${topic}`)
  const a = sentence && sentence.length >= 25 && sentence.length <= 258
    ? sentence
    : 'يظهر الأثر حين تتغيّر ممارسةٌ يمكن ملاحظتها ومراجعتها، لا حين تكثر الأدوات والمصطلحات بلا تحسّنٍ في الفهم.'
...
async function run() {
  ...
  if (SELF_TEST) {
    ...
    const letterSamples = prose.slice(0, 60).map((source, index) => fallbackLetter(source, styles[index % styles.length]))
    const uniqueLetters = new Set(letterSamples.map((letter) => letter.message)).size
    if (uniqueLetters < letterSamples.length) throw new Error(`رسائل مكرّرة: ${uniqueLetters}/${letterSamples.length}`)

    const faqSamples = prose.slice(0, 60).map((source, index) => fallbackFaq(source, topicFamilies[index % topicFamilies.length]))
    const uniqueQuestions = new Set(faqSamples.map((faq) => faq.q)).size
    const uniqueAnswers = new Set(faqSamples.map((faq) => faq.a)).size
    if (uniqueQuestions < 24) throw new Error(`أسئلة قليلة التنويع: ${uniqueQuestions}/60`)
    if (uniqueAnswers < 50) throw new Error(`أجوبة قليلة التنويع: ${uniqueAnswers}/60`)

    /* ★ ولا يُنسب إلى الدكتور ما لم يكتبه: كل جوابٍ مقتبس يجب أن يوجد
       حرفاً بحرف في متن مصدره — بوابةٌ على منوال بوابة البوت. */
    const stableAnswer = 'يظهر الأثر حين تتغيّر ممارسةٌ يمكن ملاحظتها ومراجعتها، لا حين تكثر الأدوات والمصطلحات بلا تحسّنٍ في الفهم.'
    faqSamples.forEach((faq, index) => {
      if (faq.a === stableAnswer) return
      if (!clean(prose[index].text).includes(faq.a)) {
        throw new Error(`★ جوابٌ منسوبٌ للدكتور غير موجود في متنه: «${faq.a.slice(0, 60)}…»`)
      }
    })

    /* واللقاءات لا تُقتبس: نصّها سطرٌ نصنعه نحن لا كلامٌ قاله */
    for (const interview of sources.filter((item) => item.type === 'لقاء').slice(0, 5)) {
      const letter = fallbackLetter(interview, styles[0])
      if (letter.message.includes('«لقاء بعنوان')) throw new Error('★ اقتُبس سطرٌ مصنوع كأنه كلام الدكتور')
      const faq = fallbackFaq(interview, topicFamilies[0])
      if (faq.a !== stableAnswer) throw new Error('★ جوابٌ من لقاءٍ بلا متن')
    }
    ...
    console.log(JSON.stringify({
      ...
      variety: { sampled: 60, uniqueLetters, uniqueQuestions, uniqueAnswers, engine: 'archive-grounded (no LLM)' },
      fallback: {
