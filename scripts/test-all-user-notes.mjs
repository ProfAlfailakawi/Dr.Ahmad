...
const bookToc = read('src/components/BookToc.tsx')
...
const adminNavigation = read('src/components/admin/admin-navigation.ts')
const styleChecker = read('src/components/admin/StyleChecker.tsx')
const obsoleteRootCleaner = read('scripts/clean-obsolete-root-files.mjs')
...
check('شريط المقالات ثابت داخل التدفق على الهاتف وsticky على الشاشات الأوسع', articles.includes('articles-filter-bar') && articles.includes('md:sticky md:top-16') && articles.includes('md:bg-canvas/[.96]'))
check('شريط الاستماع ثابت داخل التدفق على الهاتف وsticky على الشاشات الأوسع', listen.includes('listen-filter-bar') && listen.includes('md:sticky md:top-16') && listen.includes('md:bg-canvas/[.96]'))
...
check('تلميح تحديد الجملة يظهر مرة واحدة محلياً', articleDetail.includes('reader:selection-discovered:v2'))
...
check('ذيل المقال يحذف واجهة بعد القراءة فقط ويبقي الأدوات والتنقل وموقعها في السماء', !articleDetail.includes('>بعد القراءة</h2>') && !articleDetail.includes('امتداد الفكرة · أدوات الباحث · المشاركة والاستشهاد') && articleDetail.includes('<IdeaLife') && articleDetail.includes('<ArticleExtensions') && articleDetail.includes('مشاركة المقال والاستشهاد به') && articleDetail.includes('article-atlas-knot') && articleDetail.includes('/atlas?star=') && articleDetail.includes('aria-label="جميع المقالات"'))
...
check('فهرس الكتاب هرمي بالأبواب والفصول لا جدار بطاقات', bookToc.includes('groupToc') && bookDetail.includes('<details') && bookDetail.includes('أبوابٌ تُفتح عند الحاجة'))
check('عناوين الفهرس تنتقل إلى المحور نفسه لا إلى البحث', bookToc.includes('bookKnowledgeAnchor') && bookToc.includes('#${anchor}') && !bookToc.includes('book_question=${encodeURIComponent(entry.label)}'))
...
check('متون المقالات القديمة والجديدة تلتزم justify مع inter-word وآخر السطر من جهة البدء', css.includes('.article-body-synced') && css.includes('text-align: justify') && css.includes('text-justify: inter-word') && css.includes('text-align-last: start'))
...
check('المادتان الإذاعيتان مرتبطتان بملفي الصوت الخارجيين', mediaArchiveData.includes('\"id\": \"Tarbawyah-14\"') && mediaArchiveData.includes('\"audioFile\": \"Tarbawyah-14.mp3\"') && mediaArchiveData.includes('\"id\": \"idaa\"') && mediaArchiveData.includes('\"audioFile\": \"idaa.mp3\"'))
...
check('فلاتر النوع والجهة غير المفيدة محذوفة من الأرشيف', !media.includes('كل الجهات') && !media.includes('setKind(') && !media.includes('setOutlet('))
...
check('مشغل الأرشيف يستعمل قاعدة الصوت العامة عند غياب المتغير المتخصص', mediaDetail.includes('VITE_MEDIA_AUDIO_BASE_URL || import.meta.env.VITE_AUDIO_BASE_URL'))
...
check('إشارة المقال متعددة بهدوء وتغني كلياً عن نبض المقال', articleSignal.includes('article-pull-quote--marker-only') && articleSignal.includes('articleSignalsOf') && articleSignal.includes("source: 'readers'") && articleSignal.includes("source: 'pivot'") && articleSignal.includes("source: 'text'") && articleDetail.includes('articleSignalsOf(slug, body, popularQuotes)') && articleDetail.includes('articleSignals.filter((signal) => signal.paragraph === pIdx)') && !resonance.includes('ArticlePulse') && !articleSignal.includes('نبض المقال') && !articleDetail.includes('ArticlePulse'))
check('موسوعة تكنولوجيا التعليم لها بوابة مستقلة عن قالب الكتب العام', bookDetail.includes("book.slug === 'encyclopedia'") && bookDetail.includes('<LazyEncyclopediaPortal') && encyclopediaPortal.includes('بوابة معرفية مستقلة'))
...
check('نسخة الكويت تظهر من سجل R2 الموثق بلا فحص CORS هش', extras.includes("meta?.validationStatus === 'verified-r2'") && extras.includes('await audioMetaReady') && listen.includes('kuwaitiReady={KUWAITI_SLUGS.has(episode.slug)}'))
check('مجلس الفكرة يفتح النسخة المختارة ويعرض الكويتية بوضوح', listen.includes("dialogueVariantKey(variant)") && listen.includes('كويتي · {KUWAITI_SLUGS.size}') && listen.includes("variant === 'kuwaiti' ? episodeKwSrc"))
check('مدة الحلقة في المجلس تتبع النسخة الكويتية لا مدة الفصحى', listen.includes('KUWAITI_DURATIONS') && listen.includes('durationSec: KUWAITI_DURATIONS.get(episode.slug)'))
check('خيارات الحوار تسمي فصحى وكويتي ولا تتنكر كصوت رجل', audioPlayer.includes("const dialogueChoice = item.avatar === 'dialogue'") && audioPlayer.includes('<span>{item.label}</span>'))
...
check('مساحتي على الهاتف لا تتمدّد إلى فراغ أبيض وتبقي التمرير للمحتوى الطويل', mySpace.includes('my-space-body min-h-0 overflow-y-auto overscroll-contain md:flex-1') && mySpace.includes('px-6 py-7 text-center md:px-12 md:py-14') && css.includes('.my-space-body { flex: 0 1 auto; min-block-size: 0; }'))
check('نص الإذاعة يتبع currentTime مباشرة بلا تأخير نصف ثانية', radio.includes('audio.currentTime + 0.08') && radio.includes('setInterval(syncLine, playing ? 90 : 250)') && !radio.includes('}, 520)') && read('src/pages/radio.css').includes('@keyframes radio-line-enter'))
check('إشارة المقال تحتفظ بعقد الحارس بعد تحويلها إلى Pull Quote', articleSignal.includes('article-signal-mark article-pull-quote__action'))
...
check('فاحص الأسلوب مدمج في لوحة الإدارة الحية لا في نسخة جذر وهمية', adminPage.includes("import('../components/admin/StyleChecker')") && adminPage.includes("'style-checker': <StyleChecker") && adminNavigation.includes("tab: 'style-checker'") && styleChecker.includes('data-style-checker="true"'))
check('نسخ Google Studio الجذرية تُزال قبل كل بناء ولا تخدع الرفع التالي', ['Admin.tsx', 'StyleChecker.tsx', 'admin-navigation.ts'].every((file) => obsoleteRootCleaner.includes(`'${file}'`)))
