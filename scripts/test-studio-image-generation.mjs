...
assert.match(prompt, /REQUIRED VISIBLE SCENE/i)
assert.match(prompt, /NON-NEGOTIABLE RELEVANCE ANCHORS/i)
assert.match(prompt, /no typography/i)
assert.match(prompt, /one art family among many/i)
...
assert.match(manipulationPrompt, /educational standards/i)
assert.match(manipulationPrompt, /Reject completely:.*person hiding behind a book/i)
