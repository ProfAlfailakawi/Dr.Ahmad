...
export function ResearchSectionNavigator({ active, onSelect }: { active: ResearchLayer; onSelect: (layer: ResearchLayer) => void }) {
  return (
    <div className="research-section-navigator sticky z-[35] -mx-4 mb-6 mt-4 bg-canvas px-4 pb-4 pt-2 sm:-mx-6 sm:px-6 md:-mx-11 md:px-11">
      <nav className="mx-auto max-w-[960px] rounded-2xl border border-hair bg-paper p-1.5 shadow-sm" aria-label="التنقل داخل البحث">
        ...
        {levels.map((level, index) => (
          <button key={level.key} type="button" onClick={() => onSelect(level.key)} aria-current={active === level.key ? 'step' : undefined} className={`min-h-[3.55rem] rounded-xl border border-transparent px-2 py-2.5 text-center transition-[background-color,color,border-color] sm:px-4 ${active === level.key ? 'border-accent bg-accent text-white' : 'text-soft hover:bg-wash hover:text-ink'}`}>
