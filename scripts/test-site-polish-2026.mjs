...
ok(generationLibrary.includes('التصاميم الكاملة') && generationLibrary.includes('الأصول البصرية المولّدة') && generationLibrary.includes('فتح للتعديل'), 'مكتبة التوليد تفصل التصاميم الكاملة عن الأصول وتعيد فتحها للتحرير')
...
ok(studio.includes('backfillGenerationLibrary') && generationLibrary.includes('استعادة المحفوظات القديمة') && studio.includes('legacy-design-') && studio.includes('legacy-image-'), 'المكتبة تستعيد بأمان المحفوظات المحلية القديمة والأصول المضمّنة بمعرّفات ثابتة غير مكررة')
