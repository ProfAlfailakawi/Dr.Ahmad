...
/* كل كلمةٍ شكا منها الدكتور (٢٠ أغسطس ٢٠٢٦) مع جملتها الحاملة من المتن.
   ...
   لا كلمة مخترعة هنا. */
const WORDS = [
  /* الجولة الثالثة (٢٠ أغسطس ٢٠٢٦). سؤالان لا واحد:
     (أ) هل التشكيل الكامل يثبّت النطق؟ «ضيّج» خرجت ممتازةً في تشغيلةٍ وسيئةً في
         التالية والمدخل واحد — فالتذبذب هو العدوّ الآن لا المعجم. المقطع ١ يعيد
         الجملة نفسها ثلاثاً: مرةً كما هي، ومرتين بتشكيلٍ متدرّج.
     (ب) بدائل لكلماتٍ ردّها بأذنه: «يعرفها» و«يُدخل» خرجتا عُمانيّتين، و«رقم»
         عادت إلى قلقلتها. */
  { key: 'ضيّج — هل يثبّتها التشكيل', speaker: 'Fahad',
    carrier: 'مثل واحد كان محشور بباب {W} وطلع منه.',
    options: ['ضيّج', 'ضَيِّج', 'ظَيِّج'] },
  { key: 'يدريها — رددت يعرفها', speaker: 'Fahad',
    carrier: '{W} بعقله، إي. بس الإحساس مو دايما يسمع كلام العقل.',
    options: ['فاهمها', 'عارفها', 'يدري فيها'] },
  { key: 'رقم — القاف الملقلقة', speaker: 'Noura',
    carrier: 'يعني الدرجة اللي هي مجرد {W}… تصير حكم، ترى.',
    options: ['رقم', 'رَقَم', 'رگم'] },
  { key: 'يدخل — خرجت عمانية', speaker: 'Noura',
    carrier: 'يعني القلق مو بس يظايج الطالب؛ {W} معاه إلى داخل الأداء نفسه.',
    options: ['يدخل', 'يِدْخَل', 'يوصل'] },
]
