...
check(results.includes('مادة تعريفية للموسوعة') && results.includes('لماذا ظهرت هذه النتيجة؟') && results.includes('بلا كلام'), 'واجهة النتائج تعرض الحالات والتفسير بهدوء')
check((results.match(/لم يوجد مقطع زمني موثوق/g) || []).length === 1, 'رسالة غياب التوقيت لا تتكرر بصرياً')
check(results.includes('#page=${Math.max') && results.includes('target="_blank"') && results.includes('encyclopediaSlideRangeLabel'), 'صفحة الكتاب والشرائح تعملان من البطاقة الموحدة')
