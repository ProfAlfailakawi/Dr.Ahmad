 ...
 * قراءةُ فهد، وقراءةُ نورة، وحلقةُ الحوار — ويقول ما تمّ وما بقي وكم يلزم.
 *
 * الاستعمال:
...
/** الملفات الثلاثة المتوقَّعة لكل مقال */
...
export function articleState(found) {
  ...
  return {
    reading, dialogue, fahed, noura,
    done: reading && dialogue,
    label: reading && dialogue ? 'مكتمل' : reading ? 'قراءةٌ بلا حوار' : dialogue ? 'حوارٌ بلا قراءة' : fahed || noura ? 'قراءةٌ ناقصة' : 'لم يبدأ',
  }
}
...
if (SELF_TEST) {
  ...
  t('ثلاثة ملفات لكل مقال', expectedFiles('x').length === 3)
  t('واثنان بلا حوار', expectedFiles('x', { withDialogue: false }).length === 2)
  t('وأسماؤها بالصيغة التي يرفعها المحرّك', expectedFiles('x').join() === 'x.mp3,x.noura.mp3,x.dialogue.mp3')

  t('★ المكتمل يحتاج الصوتين والحوار', articleState([1, 1, 1]).done === true)
  t('★ صوتٌ واحد ليس قراءةً — الناس تسمع الاثنين', articleState([1, 0, 1]).reading === false)
  t('والقراءة بلا حوار تُسمّى باسمها', articleState([1, 1, 0]).label === 'قراءةٌ بلا حوار')
  t('ولم يبدأ يُقال صراحةً', articleState([0, 0, 0]).label === 'لم يبدأ')
  t('والناقص ليس مكتملاً', articleState([1, 1, 0]).done === false)
...
const partial = rows.filter((r) => !r.reading && (r.fahed || r.noura))
const none = rows.filter((r) => !r.fahed && !r.noura && !r.dialogue)

const missingReadings = rows.reduce((sum, r) => sum + (r.fahed ? 0 : 1) + (r.noura ? 0 : 1), 0)
...
if (SHOW_MISSING) {
  const show = (title, list) => {
    ...
    for (const r of list) console.log(`   · ${r.slug}${r.fahed !== r.noura ? `  [${r.fahed ? 'ينقص نورة' : 'ينقص فهد'}]` : ''}`)
    console.log()
  }
  show('لم يبدأ', none)
  show('قراءةٌ ناقصة', partial)
