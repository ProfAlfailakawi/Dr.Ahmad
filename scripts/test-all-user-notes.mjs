...
check('واجهة البحث تشرح فهم السؤال وتقترح إعادة صياغته', search.includes('فهم البحث') && search.includes('حوّلها إلى إجابة موثقة') && search.includes('بحث بالمعنى لا بالنص'))
...
check('بوابة الموسوعة تفهرس الفيديوهات وتربطها بالأبواب والفصول والبحث والعروض', encyclopediaPortal.includes('خريطة الموسوعة') && encyclopediaPortal.includes('الموسوعة المرئية') && encyclopediaPortal.includes('بحث موحّد') && encyclopediaPortal.includes('حقيبة تدريس الموسوعة') && encyclopediaPortal.includes('getEncyclopediaVideoCatalog') && encyclopediaPortal.includes('searchResults.videos') && encyclopediaPortal.includes('searchResults.slides') && encyclopediaPortal.includes('اقرأ الموضوع في الكتاب') && encyclopediaPortal.includes('teachingMaterial.door.presentation') && encyclopediaPortal.includes('مواد التدريس') && encyclopediaPortal.includes('خيط المادة') && encyclopediaPortal.includes('موضع الموضوع في العرض') && encyclopediaPortal.includes('getEncyclopediaTeachingTopic') && encyclopediaPortal.includes('onOpenTeaching') && encyclopediaPortal.includes('aria-modal="true"') && !encyclopediaPortal.includes('bg-ink/45') && !encyclopediaPortal.includes('text-soft/55') && !encyclopediaPortal.includes('FEATURED_VIDEO_EMBED') && !encyclopediaPortal.includes('channelSearch(') && encyclopediaVideoIndex.includes('extractEncyclopediaSequence') && encyclopediaVideoIndex.includes('resolveConcept') && encyclopediaVideoClient.includes('/api/encyclopedia/videos') && encyclopediaVideoServer.includes('parseYouTubeContinuations') && encyclopediaTeachingMap.includes('encyclopediaSlideRangeLabel') && nodeServer.includes('loadEncyclopediaVideoCatalog'))
...
check('عروض أبواب الموسوعة الأربعة مضمّنة كملفات تحميل فعلية', [1, 2, 3, 4].every((door) => exists(`public/files/encyclopedia/encyclopedia-door-${door}.pptx`)) && encyclopediaPortal.includes('download={`موسوعة تكنولوجيا التعليم - الباب ${door.number}.pptx`}'))
