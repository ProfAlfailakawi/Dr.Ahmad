 ...
 *
 * كل عملية بناء تُولّد معرّفاً فريداً: طابع زمني + هاش قصير. هذا المعرّف:
 ...
 *      الخدمة مع كل إصدار، فلن يرى المتصفح تحديثاً أصلاً ولن تعلم التبويبات المفتوحة بشيء.
 *
 ...
 *   serviceWorkers: أسماء ملفات العامل داخل dist التي يجب أن تحمل البصمة (افتراضياً sw.js).
 */
export function buildStamp(options = {}) {
  ...
  return {
    ...
    closeBundle() {
      ...
      // بايتات عامل الخدمة يجب أن تختلف مع كل إصدار، وإلا لم يرَ المتصفح تحديثاً.
